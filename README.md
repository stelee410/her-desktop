...
Successful plugin installs return a quick-start summary with each capability's function name, native form inputs, adapter type, and approval requirement, so vibe-coded extensions are immediately usable from the Plugin Library or model tool calls. When a draft is already staged, conversation can continue with `plugin.installDraft` or `plugin.discardDraft` by plugin id instead of asking the model to reconstruct package JSON.
