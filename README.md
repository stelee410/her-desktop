...
Recent local tool results are reintroduced into the next model turn as bounded "tool result evidence" snippets, not trusted instructions. Approved capability follow-ups inject the fresh approved result separately so the model can continue from the real executor output without duplicating stale tool blobs.
