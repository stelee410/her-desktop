...
The Swift fallback built-ins only keep the core runtime usable when processed resources are unavailable; they are not the source of truth for adding new built-ins.
...
The Vibe Plugin Composer can call `tools/list` on a local MCP bridge, show discovered tools and input fields, fill `methodName=tools/call` plus `toolName` from a selected tool, directly draft or install a local MCP plugin from a discovered tool, and carry supported discovered schema fields into the generated plugin `inputSchema`. Installed local plugins can also be sent back into the composer from the Plugin Library or Tools workspace as AI update drafts; Her Desktop carries the plugin id, capability summary, package file list, and redacted key file excerpts into AgentLLM so the result is a complete replacement package that returns to the same review/install flow.
...
When the user asks to generate and install an extension in one conversational flow, `plugin.draft` accepts `install_immediately`; Her Desktop still stages the generated package for review and queues the approved `plugin.installDraft` step rather than silently enabling new code.
...
Pasted PluginPackage JSON or a selected `.plugin-package.json` file is staged into the same generated-drafts review queue before installation, either from the UI composer or through the `plugin.stagePackage` capability. The composer can also import a local text/Markdown skill file and wrap it as a skill-backed PluginPackage with `SKILL.md`, generated review documentation, validation, and the same install/discard flow.
