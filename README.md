...
The Vibe Plugin Composer can call `tools/list` on a local MCP bridge, show discovered tools and input fields, fill `methodName=tools/call` plus `toolName` from a selected tool, and carry supported discovered schema fields into the generated plugin `inputSchema`.
