...
final class CapabilityRuntimeTests: XCTestCase {
    ...
    func testBuiltInToolCatalogUsesManifestOwnedSchemas() throws {
        ...
        let listInstalled = try XCTUnwrap(toolFunction(named: "plugin_listInstalled", in: catalog))
        let listInstalledParameters = try XCTUnwrap(listInstalled["parameters"] as? [String: Any])
        let listInstalledProperties = try XCTUnwrap(listInstalledParameters["properties"] as? [String: Any])
        ...
        let discardDraft = try XCTUnwrap(toolFunction(named: "plugin_discardDraft", in: catalog))
        let discardDraftParameters = try XCTUnwrap(discardDraft["parameters"] as? [String: Any])
        let discardDraftProperties = try XCTUnwrap(discardDraftParameters["properties"] as? [String: Any])

        XCTAssertEqual((discardDraftProperties["plugin_id"] as? [String: Any])?["type"] as? String, "string")
    ...
    func testBuiltInPluginListInstalledDoesNotRequireApproval() {
        let registry = PluginRegistry(config: .empty)
        let capability = registry.capability(id: "plugin.listInstalled")
