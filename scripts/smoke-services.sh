...
CONFIG_FILE="$(mktemp)"
LLM_CURL_CONFIG="$(mktemp)"
MEM_CURL_CONFIG="$(mktemp)"
trap 'rm -f "$CONFIG_FILE" "$LLM_CURL_CONFIG" "$MEM_CURL_CONFIG"' EXIT
chmod 600 "$CONFIG_FILE" "$LLM_CURL_CONFIG" "$MEM_CURL_CONFIG"

ROOT="$ROOT" CONFIG_FILE="$CONFIG_FILE" python3 - <<'PY'
import json
import os
...
for path in paths:
    try:
        Path(os.environ["CONFIG_FILE"]).write_text(path.read_text())
        break
...
printf 'header = "Authorization: Bearer %s"\n' "$HER_AGENT_LLM_API_KEY" > "$LLM_CURL_CONFIG"
if [[ -n "$HER_AGENT_MEM_API_KEY" ]]; then
  printf 'header = "X-Memory-API-Key: %s"\n' "$HER_AGENT_MEM_API_KEY" > "$MEM_CURL_CONFIG"
fi
...
echo "AgentLLM chat"
curl_retry --config "$LLM_CURL_CONFIG" --http1.1 --connect-timeout 10 --max-time 45 --retry 3 --retry-all-errors --retry-delay 1 -fsS "$LLM_BASE/v1/chat/completions" \
...
curl_retry --config "$MEM_CURL_CONFIG" --http1.1 --connect-timeout 10 --max-time 30 --retry 3 --retry-all-errors --retry-delay 1 -fsS "$MEM_BASE/v1/memory/relationship" \
...
curl_retry --config "$MEM_CURL_CONFIG" --http1.1 --connect-timeout 10 --max-time 30 --retry 3 --retry-all-errors --retry-delay 1 -fsS "$MEM_BASE/v1/memory/emotion" \
...
query_http_code="$(curl_retry --config "$MEM_CURL_CONFIG" --http1.1 --connect-timeout 10 --max-time 45 --retry 5 --retry-all-errors --retry-delay 2 -sS -o "$query_body_file" -w "%{http_code}" "$MEM_BASE/v1/memory/query" \
  -H "Content-Type: application/json" \
  -d "$QUERY_BODY" || true)"
...
if [[ "${HER_SMOKE_WRITE_MEMORY:-0}" == "1" ]]; then
  ...
  add_http_code="$(curl_retry --config "$MEM_CURL_CONFIG" --http1.1 --connect-timeout 10 --max-time 45 --retry 5 --retry-all-errors --retry-delay 2 -sS -o "$add_body_file" -w "%{http_code}" "$MEM_BASE/v1/memory/add" \
    -H "Content-Type: application/json" \
