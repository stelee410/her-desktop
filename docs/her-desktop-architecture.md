...
flowchart LR
    ...
    subgraph implementation["Her Desktop V1 实现落点"]
        ...
        native2["Native Adapter<br/>notifications, file read, attachment inspect, speak"]
...
- 生成的插件包必须可审核、可持久化、可安装、可导出；已暂存草稿必须能通过 approved `plugin.installDraft` capability 对话式安装、通过 approved `plugin.discardDraft` capability 对话式丢弃，本地插件也必须能通过 approved `plugin.remove` capability 对话式移除。
