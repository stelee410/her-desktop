...
1. 在浏览器抽屉切到「日常 Chrome」，点「导出扩展并打开文件夹」（会把扩展复制到
   `~/Documents/Her Desktop Browser Extension/` 并在访达选中——用可见目录是因为
   Chrome 的文件选择器进不去隐藏的 `.her`）。
2. Chrome → `chrome://extensions` → 打开右上「开发者模式」→「加载已解压的扩展程序」
   → 选「Her Desktop Browser Extension」文件夹。
